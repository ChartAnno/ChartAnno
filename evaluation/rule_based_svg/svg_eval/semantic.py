"""Map diffed SVG raw objects to the seven ChartAnno semantic categories."""
...
def _feature_from_record(record: dict) -> dict:
    item = {
        "src": f"svg_{record.get('semantic_role') or record.get('tag') or record.get('kind')}",
    }
    for key in (
        "ax_index",
        "bbox",
...
def _collapse_svg_text_glyphs(records: Iterable[dict]) -> list[dict]:
    grouped: dict[str, list[dict]] = {}
    passthrough: list[dict] = []
    for record in records:
        is_glyph = record.get("type") == "SVGUseElement" or record.get("tag") == "use"
        group_id = str(record.get("native_group_id") or "")
        if is_glyph and group_id.startswith("text_"):
            grouped.setdefault(group_id, []).append(record)
        else:
            passthrough.append(record)
    for group_id, items in grouped.items():
        first = dict(items[0]); first["kind"] = "text"; first["tag"] = "use"; first["native_group_id"] = group_id
        for key in ("bbox", "axes_bbox"):
            boxes = [r.get(key) for r in items if isinstance(r.get(key), (list, tuple)) and len(r.get(key)) == 4]
            if boxes:
                x0 = min(float(b[0]) for b in boxes); y0 = min(float(b[1]) for b in boxes)
                x1 = max(float(b[0]) + float(b[2]) for b in boxes); y1 = max(float(b[1]) + float(b[3]) for b in boxes)
                first[key] = [x0, y0, x1 - x0, y1 - y0]
...
def _infer_feature_key(record: dict) -> str | None:
    ...
    if kind == "annotation_arrow":
        return "2_connector"
    if record.get("type") == "SVGUseElement" or tag == "use":
        return "3_text"
    if kind == "axes":
        return "7_geometric"
    if kind == "line":
...
def classify_unmatched_records(records: Iterable[dict]) -> dict[str, list]:
    records = _collapse_svg_text_glyphs(list(records))
...
def append_matched_color_diffs(semantic: dict[str, list], matched_pairs: Iterable[tuple[dict, dict]]) -> None:
    ...
    for candidate, baseline in matched_pairs:
        ...
        is_text_glyph = (
            candidate.get("kind") == "text"
            or str(candidate.get("native_group_id") or "").startswith("text_")
            or candidate.get("type") == "SVGUseElement"
        )
        if is_text_glyph:
            item["src"] = "svg_text_color_change"
            semantic["3_text"].append(item)
        else:
            item["src"] = f"svg_{candidate.get('kind')}_color_change"
