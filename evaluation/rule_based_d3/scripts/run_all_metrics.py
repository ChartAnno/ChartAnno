#!/usr/bin/env python3
"""Run D3-native GT/prediction rule-based evaluation."""
...
from d3_eval.metrics import compute_rule_based_metrics  # noqa: E402
...
def _chart_relative(prediction: Path, prediction_root: Path, gt_root: Path) -> Path:
    ...
    stem = Path(category) / match.group(1)
    for suffix in (".html", ".js"):
        if (gt_root / stem.with_suffix(suffix)).is_file():
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description=__doc__,
        epilog=(
            "Jaccard/Color compare (D3 GT - D3 removed) with "
            "(D3 prediction - D3 removed)."
        ),
    )
    parser.add_argument("--gt-root", default=str(assets / "dataset_code_D3"), help="Real D3 ground-truth root")
    parser.add_argument("--baseline-root", default=str(assets / "dataset_removed_D3"), help="D3 removed/baseline root")
    parser.add_argument("--test-root", default=str(assets / "test_code_D3"), help="D3 prediction root")
    parser.add_argument("--output-root", default=str(TOOL_ROOT / "outputs"))
    parser.add_argument("--sample", action="append", help="Relative prediction HTML path; may be repeated")
    ...
    predictions = (
        ...
        if args.sample
        else sorted([*prediction_root.rglob("*.html"), *prediction_root.rglob("*.js")])
    )
    if not predictions:
        raise SystemExit(
            f"No D3 predictions found under {prediction_root}. Formal metrics require "
            "real D3 GT, D3 test outputs, and D3 removed baselines."
        )
    ...
    with D3BrowserRuntime() as runtime:
        for prediction in predictions:
            ...
            gt_bundle = extract_diffed_d3_bundle(gt_candidate, baseline, runtime=runtime)
            prediction_bundle = extract_diffed_d3_bundle(
                prediction,
                baseline,
            ...
            runtime.render_bbox_overlay(
                ...
                title="D3 prediction diffed semantic bboxes",
            )
    ...
    summary = {
        "backend": "d3-svg",
        "mode": "evaluation",
        "comparison": "D3 GT diff vs D3 prediction diff; both use the same D3 removed baseline",
        **_aggregate(results),
        "files": results,
    }
